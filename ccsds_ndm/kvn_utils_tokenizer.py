...
"""
KVN tokeniser: line classification for KVN file I/O.

Converts a raw KVN string into an ordered list of typed
:class:`KvnLine` objects.  Each line is classified into one of:

- :class:`BlankLine` — empty or whitespace-only
- :class:`SectionMarkerLine` — ``META_START``, ``DATA_STOP``, etc.
- :class:`CommentLine` — ``COMMENT text``
- :class:`KvLine` — ``KEY = value [unit]``
- :class:`TdmObsLine` — ``KEY = EPOCH  value`` (TDM data lines)
- :class:`PackedDataLine` — epoch-led space-separated record (OEM/AEM)
- :class:`CovarianceRowLine` — space-separated numeric row (OEM covariance)

The public entry point is :func:`tokenize`.
"""
...
class KvLine(KvnLine):
    ...
    def to_str(self, key_width: int = 24) -> str:
